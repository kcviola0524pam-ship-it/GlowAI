...
const navItems = [
  ...
  { id: 'ratings', label: 'My Ratings & Reviews' },
];

export default function StaffNav({ active, onChange }) {
  const [isOpen, setIsOpen] = useState(false);
  const { darkMode, getPortalColors, logo } = useTheme();
  const customColors = getPortalColors('staff');
    ...
    <>
      ...
      >
        ...
        <div className="mb-6">
          ...
          <div>
            <p className="text-xs sm:text-sm uppercase tracking-wide font-bold" style={{ color: darkMode ? customColors.darkTextColor : customColors.textColor }}>Staff Portal</p>
